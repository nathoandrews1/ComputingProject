...
describe('AES Message Decryption', function () {
  this.timeout(0);
  let driver;
  ...
  it('Decrypt a text message with AES', async function () {
    ...
    await driver.findElement(By.id("message")).sendKeys(encryptedResult);
    await driver.findElement(By.id("key")).clear();
    await driver.findElement(By.id("key")).sendKeys(key);
    await driver.findElement(By.id("decryptBtn")).click();
    const decryptedResult = await driver.findElement(By.id("decrypted")).getText();
    expect(decryptedResult).to.equal(message);
  });
});
